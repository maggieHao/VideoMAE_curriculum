...
def train_class_batch_mask(model, samples, target, criterion, threshold = 0):
    outputs = model(samples)
    pred_index = outputs.argmax(dim=1)
    target_index = target.argmax(dim=1)
    # print(f'pred_index shape as {pred_index.shape}.')
    # print(f'target_index shape as {target_index.shape}.')
    if threshold == 0:
...
def monitor_resources(step):
    print(f"Step {step} 资源使用情况:")
    print(f"GPU显存: {torch.cuda.memory_allocated()/1024**2:.2f}MB")
    print(f"GPU显存峰值: {torch.cuda.max_memory_allocated()/1024**2:.2f}MB")
    print(f"缓存显存: {torch.cuda.memory_reserved()/1024**2:.2f}MB")

def train_one_epoch(index_remove, model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets, index, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
...
def train_one_epoch_mask(threshold, model: torch.nn.Module, criterion: torch.nn.Module,
                    data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, loss_scaler, max_norm: float = 0,
                    model_ema: Optional[ModelEma] = None, mixup_fn: Optional[Mixup] = None, log_writer=None,
                    start_steps=None, lr_schedule_values=None, wd_schedule_values=None,
