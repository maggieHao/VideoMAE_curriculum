...
from engine_for_finetuning import train_one_epoch, train_one_epoch_old, train_one_epoch_mask, validation_one_epoch, final_test, merge
...
def get_args():
    ...
    parser.add_argument('--mask_curriculum_threshold', type=int, default=0)
...
def main(args, ds_init):
    ...
    # train wrong samples
    index_length = len(dataset_train)

    for epoch in range(args.start_epoch, args.epochs):
        index_remove_original = torch.zeros(index_length, device=device)
        if args.distributed:
        ...
        # torch.distributed.barrier()
        # dataset_size = torch.tensor(len(dataset_train), device=device)
        # torch.distributed.all_reduce(dataset_size)
        ...
        num_training_steps_per_epoch = len(dataset_train) // total_batch_size
        print(f"length of data loader train is: {len(data_loader_train)}")
        print(f"num_training_steps_per_epoch is: {num_training_steps_per_epoch}")
        print("Change step level LR scheduler!")
        lr_schedule_values = utils.cosine_scheduler(
            args.lr, args.min_lr, args.epochs, num_training_steps_per_epoch,
            warmup_epochs=args.warmup_epochs, warmup_steps=args.warmup_steps,
        )
...
def main_baseline(args, ds_init):
    utils.init_distributed_mode(args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if log_writer is not None:
            log_writer.set_step(epoch * num_training_steps_per_epoch * args.update_freq)
        train_stats = train_one_epoch_old(
            model, criterion, data_loader_train, optimizer,
            device, epoch, loss_scaler, args.clip_grad, model_ema, mixup_fn,
            log_writer=log_writer, start_steps=epoch * num_training_steps_per_epoch,
            lr_schedule_values=lr_schedule_values, wd_schedule_values=wd_schedule_values,
...
def main_mask_curriculum(args, ds_init):
    utils.init_distributed_mode(args)
    ...
    start_time = time.time()
    max_accuracy = 0.0
    print("train curriculum learning mask.") 
    print("Mask curriculum threshold = %s" % str(args.mask_curriculum_threshold))
    for epoch in range(args.start_epoch, args.epochs):
        if args.distributed:
            data_loader_train.sampler.set_epoch(epoch)
        if log_writer is not None:
            log_writer.set_step(epoch * num_training_steps_per_epoch * args.update_freq)
        train_stats = train_one_epoch_mask(
            args.mask_curriculum_threshold/100,
            model, criterion, data_loader_train, optimizer,
            device, epoch, loss_scaler, args.clip_grad, model_ema, mixup_fn,
            log_writer=log_writer, start_steps=epoch * num_training_steps_per_epoch,
            lr_schedule_values=lr_schedule_values, wd_schedule_values=wd_schedule_values,
...
if __name__ == '__main__':
    ...
    # main(opts, ds_init)
    # main_baseline(opts, ds_init)
    main_mask_curriculum(opts, ds_init)
